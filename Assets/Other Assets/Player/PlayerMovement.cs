...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update() // runs on every frame
    {
        ...
        if (wallJumpCooldown > 0.2f)
        {
            ...
            body.linearVelocity = new Vector2(horizontalInput * speed, body.linearVelocity.y); // vector is collection of numbers, for speed and direction - horizontal will go from -1 to 1

            if(onWall() && !isGrounded()) // checks if player is on the wall and not grounded
            {
                body.gravityScale = 0;
                body.linearVelocity = Vector2.zero; // if player jumps on wall, becomes 'stuck'
            }
            else
            {
    ...
    private void Jump()
    {
        ...
        else if(onWall() && !isGrounded()) // checks if player is on the wall and not grounded 
        {
            ...
            {
                body.linearVelocity = new Vector2(-Mathf.Sign(transform.localScale.x) * 2, 6);
